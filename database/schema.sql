...
GO
CREATE UNIQUE INDEX UX_Addresses_Default ON Addresses(UserID) WHERE IsDefault = 1;
GO
...
CREATE TABLE Products (
    ...
    CONSTRAINT CK_Products_ConnectionType CHECK (ConnectionType IS NULL OR ConnectionType IN ('WIRED','WIRELESS','BLUETOOTH')),
    CONSTRAINT CK_Products_DriverSize CHECK (DriverSize IS NULL OR DriverSize > 0),
    CONSTRAINT CK_Products_BatteryLife CHECK (BatteryLife IS NULL OR BatteryLife >= 0),
...
CREATE TABLE InventoryTransactions (
    ...
    CONSTRAINT CK_InventoryTransactions_Type CHECK (TransactionType IN ('IMPORT','SALE','RETURN','ADJUSTMENT')),
    CONSTRAINT CK_InventoryTransactions_Quantity CHECK (Quantity > 0),
    CONSTRAINT CK_InventoryTransactions_Previous CHECK (PreviousQuantity >= 0),
    CONSTRAINT CK_InventoryTransactions_New CHECK (NewQuantity >= 0)
);
GO
...
CREATE TABLE Orders (
    ...
    CONSTRAINT CK_Orders_Status CHECK (OrderStatus IN ('PENDING','CONFIRMED','SHIPPING','DELIVERED','CANCELLED'))
);
GO
...
CREATE TABLE Payments (
    ...
    CONSTRAINT CK_Payments_Method CHECK (PaymentMethod IN ('COD','VNPAY','BANK_TRANSFER')),
    CONSTRAINT CK_Payments_Amount CHECK (Amount >= 0),
    CONSTRAINT CK_Payments_Status CHECK (PaymentStatus IN ('PENDING','SUCCESS','FAILED','CANCELLED'))
);
GO
...
CREATE TABLE Shipments (
    ...
    CONSTRAINT CK_Shipments_Status CHECK (ShippingStatus IN ('PENDING','PROCESSING','SHIPPING','DELIVERED','FAILED'))
);
GO
...
CREATE TABLE Vouchers (
    ...
    CreatedBy INT,
    CreatedAt DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_Vouchers_Users FOREIGN KEY (CreatedBy) REFERENCES Users(UserID),
    ...
    CONSTRAINT CK_Vouchers_MinOrder CHECK (MinOrderAmount >= 0),
    CONSTRAINT CK_Vouchers_MaxDiscount CHECK (MaxDiscount IS NULL OR MaxDiscount >= 0),
...
CREATE TABLE OrderVouchers (
    ...
    CONSTRAINT CK_OrderVouchers_Discount CHECK (DiscountAmount >= 0),
    CONSTRAINT UQ_OrderVouchers_Order UNIQUE (OrderID)
);
GO
...
CREATE TABLE UserBehaviors (
    ...
    CreatedAt DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_UserBehaviors_Users FOREIGN KEY (UserID) REFERENCES Users(UserID) ON DELETE SET NULL,
    CONSTRAINT FK_UserBehaviors_Products FOREIGN KEY (ProductID) REFERENCES Products(ProductID),
    CONSTRAINT CK_UserBehaviors_Type CHECK (BehaviorType IN ('VIEW','SEARCH','FAVORITE','CART','PURCHASE'))
);
GO
...
CREATE TABLE AIRecommendations (
    ...
    CreatedAt DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_AIRecommendations_Users FOREIGN KEY (UserID) REFERENCES Users(UserID) ON DELETE SET NULL,
    CONSTRAINT FK_AIRecommendations_Products FOREIGN KEY (ProductID) REFERENCES Products(ProductID),
    CONSTRAINT CK_AIRecommendations_Type CHECK (RecommendationType IN ('SIMILAR','PERSONALIZED','TRENDING','BEST_SELLER','COLD_START')),
    CONSTRAINT CK_AIRecommendations_Similarity CHECK (SimilarityScore BETWEEN 0 AND 1),
    CONSTRAINT CK_AIRecommendations_Behavior CHECK (BehaviorScore BETWEEN 0 AND 1),
    CONSTRAINT CK_AIRecommendations_Final CHECK (FinalScore BETWEEN 0 AND 1)
);
GO
...
CREATE TABLE Notifications (
    ...
    CONSTRAINT FK_Notifications_Orders FOREIGN KEY (OrderID) REFERENCES Orders(OrderID) ON DELETE SET NULL,
    CONSTRAINT CK_Notifications_Type CHECK (NotificationType IN ('ORDER','PAYMENT','DELIVERY')),
...
CREATE INDEX IX_UserBehaviors_ProductID ON UserBehaviors(ProductID);
CREATE INDEX IX_UserBehaviors_BehaviorType ON UserBehaviors(BehaviorType);
CREATE INDEX IX_UserBehaviors_CreatedAt ON UserBehaviors(CreatedAt);
CREATE INDEX IX_AIRecommendations_UserID ON AIRecommendations(UserID);
CREATE INDEX IX_AIRecommendations_SessionID ON AIRecommendations(SessionID);
CREATE INDEX IX_AIRecommendations_ProductID ON AIRecommendations(ProductID);
CREATE INDEX IX_AIRecommendations_FinalScore ON AIRecommendations(FinalScore DESC);

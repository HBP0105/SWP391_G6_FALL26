...
<head>
    ...
    <style>
        ...
        .badge-inactive { background: #fdf0f0; color: #c0392b; }
        ...
        .specs-table tr:nth-child(odd) td { background: #f9f9f9; }
        .specs-table td {
            padding: 9px 14px;
        ...
        .not-found-box i { font-size: 60px; color: #ddd; margin-bottom: 20px; display: block; }
        .not-found-box h3 { color: #888; }
    </style>
</head>
...
<body>
    ...
    <style>
        .search-suggestion-item { display:flex; align-items:center; padding:8px 12px; cursor:pointer; border-bottom:1px solid #f0f0f0; text-decoration:none; color:#333; transition:background 0.15s; }
        .search-suggestion-item:hover, .search-suggestion-item.active { background:#f5f5f5; }
        .suggestion-img { width:40px; height:40px; object-fit:cover; border-radius:4px; margin-right:10px; flex-shrink:0; border:1px solid #eee; }
        .suggestion-info { flex:1; overflow:hidden; }
        .suggestion-name { font-size:13px; font-weight:600; color:#333; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .suggestion-meta { font-size:11px; color:#999; margin-top:2px; }
        .suggestion-price { font-size:13px; font-weight:700; color:#D10024; white-space:nowrap; margin-left:8px; }
        .suggestion-no-result { padding:12px; text-align:center; color:#999; font-size:13px; }
    </style>
    <script>
    (function () {
        ...
        function fetchSuggestions(keyword) {
            if (keyword.length < 3) { hideSuggestions(); return; }
            var xhr = new XMLHttpRequest();
            xhr.open('GET', contextPath + '/searchSuggest?q=' + encodeURIComponent(keyword), true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try { renderSuggestions(JSON.parse(xhr.responseText)); } catch (e) { hideSuggestions(); }
                }
            };
            xhr.send();
        }
        ...
        searchInput.addEventListener('input', function () {
            ...
            debounceTimer = setTimeout(function () { fetchSuggestions(kw); }, 300);
        });
        ...
        searchInput.addEventListener('focus', function () {
            if (this.value.trim().length >= 3 && suggestionsBox.innerHTML) suggestionsBox.style.display = 'block';
        });
